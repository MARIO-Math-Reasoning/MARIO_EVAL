...
try:
    from gen_orig.PS_origParser import PS_origParser
    from gen_orig.PS_origLexer import PS_origLexer
    from gen_orig.PS_origListener import PS_origListener
except Exception:
    from .gen_orig.PS_origParser import PS_origParser
    from .gen_orig.PS_origLexer import PS_origLexer
    from .gen_orig.PS_origListener import PS_origListener
...
def latex2sympy(sympy: str, variable_values={}):
    ...
    lex = PS_origLexer(stream)
    lex.removeErrorListeners()
    lex.addErrorListener(matherror)

    tokens = CommonTokenStream(lex)
    parser = PS_origParser(tokens)
...
class MathErrorListener(ErrorListener):
    ...
    def syntaxError(self, recog, symbol, line, col, msg, e):
        ...
        if msg.startswith("missing"):
            ...
            names = PS_origParser.literalNames
...
def convert_frac(frac):
    ...
    if frac.lower and frac.upper:
        ...
        if (frac.lower.start == frac.lower.stop and
                frac.lower.start.type == PS_origLexer.DIFFERENTIAL):
            wrt = get_differential_var_str(frac.lower.start.text)
            diff_op = True
        elif (lower_itv_len == 2 and
              frac.lower.start.type == PS_origLexer.SYMBOL and
              frac.lower.start.text == '\\partial' and
              (frac.lower.stop.type == PS_origLexer.LETTER_NO_E or frac.lower.stop.type == PS_origLexer.SYMBOL)):
            partial_op = True
            wrt = frac.lower.stop.text
            if frac.lower.stop.type == PS_origLexer.SYMBOL:
                wrt = wrt[1:]

        if diff_op or partial_op:
            wrt = sympy.Symbol(wrt, real=is_real)
            if (diff_op and frac.upper.start == frac.upper.stop and
                frac.upper.start.type == PS_origLexer.LETTER_NO_E and
                    frac.upper.start.text == 'd'):
                return [wrt]
            elif (partial_op and frac.upper.start == frac.upper.stop and
                  frac.upper.start.type == PS_origLexer.SYMBOL and
