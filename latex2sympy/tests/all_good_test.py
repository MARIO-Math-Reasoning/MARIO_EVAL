...
class TestAllGood(object):
    ...
    GOOD_PAIRS = [
        ...
        ("\\frac{d x}{dx}", Derivative(x, x)),
        ("\\frac{d}{d t} x", Derivative(x, t)),
        # ("f(x)", f(x)),
        # ("f(x, y)", f(x, y)),
        # ("f(x, y, z)", f(x, y, z)),
        ...
        ("h_\\theta ", Symbol('h_{\\theta}')),
        ("h_{\\theta}", Symbol('h_{\\theta}')),
        # ("h_{\\theta}(x_0, x_1)", Symbol('h_{theta}', real=True)(Symbol('x_{0}', real=True), Symbol('x_{1}'))),
        ("x!", _factorial(x)),
        ("100!", _factorial(100)),
        ...
        ("2\\overline{x}_n", 2 * Symbol('xbar_n')),
        ("\\frac{x}{\\overline{x}_n}", x / Symbol('xbar_n')),
        ("\\frac{\\sin(x)}{\\overline{x}_n}", sin(Symbol('x')) / Symbol('xbar_n')),
        ("2\\bar{x}", 2 * Symbol('xbar')),
        ("2\\bar{x}_n", 2 * Symbol('xbar_n')),
        ...
        ("\\frac{1}{2}xy(x+y)", Mul(_Pow(2, -1), x, y, (x + y))),
        ("\\frac{1}{2}\\theta(x+y)", Mul(_Pow(2, -1), theta, (x + y))),
        # ("1-f(x)", 1 - f * x),
        ...
        ("\\theta\\begin{matrix}1\\\\3\\end{matrix} - \\begin{matrix}-1\\\\2\\end{matrix}", MatAdd(MatMul(theta, Matrix([[1], [3]]), evaluate=False), MatMul(-1, Matrix([[-1], [2]])), evaluate=False)),
